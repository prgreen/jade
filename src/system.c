...
int fs_listdir(const char *dir, FS_LISTDIR_CALLBACK cb, void *user)
{
	...
	do
	{
		...
		{
			str_format(join_buf,2048,"%s\\%s",dir,
							finddata.cFileName);
			cb(finddata.cFileName,
					fs_is_dir(join_buf), user);
		}
	...
	{
		str_format(join_buf,2048,"%s/%s",dir, entry->d_name);
		cb(entry->d_name, fs_is_dir(join_buf), user);
	}
